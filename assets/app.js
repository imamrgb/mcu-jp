...
function updateAutomaticMarks(data){
  ...
  ensureAutoMark('gender-male-id',false,!male);
  ensureAutoMark('gender-female-id',false,male);
...
function applyMarks(){
  document.querySelectorAll('.markable').forEach(node=>{
    const mark=state.marks[node.dataset.markKey]||{};
    node.classList.toggle('is-struck',Boolean(mark.strike));
    node.classList.toggle('is-circled',Boolean(mark.circle));
    node.classList.toggle('selected',state.selectedMark===node.dataset.markKey);
  });
}
function render(){
...
el('btnSave').addEventListener('click',saveCurrent); el('btnLoad').addEventListener('click',loadCurrent); el('btnDelete').addEventListener('click',deleteCurrent); el('btnNew').addEventListener('click',newRecord); el('btnSample').addEventListener('click',useSample); el('btnPrint').addEventListener('click',()=>window.print());
