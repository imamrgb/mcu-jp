...
(() => {
  ...
  async function loadGeminiConfig(){
    ...
    try{
      ...
      geminiRuntime.config={hasApiKey:false,primaryModel:'gemini-2.5-flash',fallbackModels:['gemini-2.5-flash-lite','gemini-2.0-flash'],allowBrowserOverride:true};
    }
    return geminiRuntime.config;
  }
  function updateModeUi(){
    const mode=sEl('scanMode').value;
    const useGemini=mode==='gemini';
    document.querySelectorAll('.local-ocr-option').forEach(node=>node.hidden=useGemini);
    const box=sEl('geminiConfigBox'); if(box) box.hidden=!useGemini;
  }
  async function applyGeminiConfigToUi(){
    const cfg=await loadGeminiConfig();
    if(cfg.primaryModel && sEl('geminiModel')) sEl('geminiModel').value=cfg.primaryModel;
    const hint=sEl('geminiConfigHint');
    if(hint) hint.innerHTML=cfg.hasApiKey ? 'API key sudah terdeteksi dari file <b>gemini.config.json</b>. Input di browser bersifat override sementara.' : 'API key belum ada di file konfigurasi. Isi <b>gemini.config.json</b> atau tempel di input ini.';
    const fb=sEl('geminiFallbackHint');
    if(fb) fb.textContent=`Jika model utama gagal atau kuota habis, aplikasi mencoba otomatis: ${(cfg.fallbackModels||[]).join(' → ') || 'tidak ada fallback'}.`;
    updateModeUi();
  }
  ...
  async function analyzeWithGemini(files){
    const cfg=await loadGeminiConfig();
    const key=String(sEl('geminiApiKey').value||'').trim();
    ...
    const model=sEl('geminiModel').value||cfg.primaryModel||'gemini-2.5-flash';
    const modelChain=[model,...(cfg.fallbackModels||[])].filter((v,i,a)=>v&&a.indexOf(v)===i);
    const response=await fetch('/api/gemini',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({apiKey:key,model,modelChain,request})});
  ...
  async function analyze(){
    ...
    try{
      const groups=groupFiles(scanState.files,sEl('scanGrouping').value),mode=sEl('scanMode').value;
  ...
  sEl('btnToggleApiKey').addEventListener('click',()=>{const input=sEl('geminiApiKey');const visible=input.type==='text';input.type=visible?'password':'text';sEl('btnToggleApiKey').textContent=visible?'Lihat':'Sembunyikan'});
  sEl('scanMode').addEventListener('change',updateModeUi);
